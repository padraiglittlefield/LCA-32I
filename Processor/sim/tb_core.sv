...
module tb_core;
    ...
    // Send an instruction to a specific FU pipe
    task send_instruction (
        ...
        begin
            ...
            // Scheduler dispatch
            dut.disp_pkt[fu_idx]        = new_disp_pkt;
            dut.disp_valid[fu_idx]      = 1'b1;
            ...
            $display("Sent instruction: OPCODE=%s, rob_entry=%0d, PC=0x%h, alu_en=%0b, imm_val=%0d, dst_areg=%0d, dst_preg=%0d, br_taken=%0b, rs_entry=%0d, rs_full=%0b at Cycle %0d",
                opcode.name(), rob_entry_idx, pc, alu_en, imm_val, dst_areg, dst_preg, br_taken,
                dut.rs_entry_idx[fu_idx], dut.rs_full[fu_idx], cycle_count);
    ...
    task test_dispatch();
        begin
            ...
            send_instruction(0, '0, ADDI, 32'h0, 0, 1, 33, 16, 1'b1, 1'b1, 1'b0);
            @(posedge clk);
        end
    endtask
